...
class ZModem {
    ...
    enum ZmodemConstants {
        ZPAD    = 0x2a,      /* 0x2a Padding character begins frames */
        ZDLE    = 0x18,         /* (␘, \x18 Zmodem escape - `ala BISYNC DLE */
        ZDLEE   = 0x58,     //(ZDLE^0x40)  /* Escaped ZDLE as transmitted */
...
protected:
    ...
    // protocol primitives
    static void sendHexHeader(int type, const char *hdr);
    static void sendData(char *buf, int length, int frameend);
    // void zsdata2(char* buf, int length, int frameend);
...
class ZMCRC32 {
private:
    static constexpr uint32_t crctab[] = { /* CRC polynomial 0xedb88320 */
...
public:
    static uint32_t crc;
    static void begin();
    static void begin(uint32_t value);

    static void update(uint8_t cp);
    static void invert();

    static uint32_t get();
    static void rightshift(uint shift);
};
